...
class EnumMapper(BaseEstimator, TransformerMixin):
    ...
    def fit(self, df,y=None):
        ...
        #Convert columns that may be numeric to numeric
        df = df.copy()
        not_number_cols_raw = df.select_dtypes(exclude=['int', 'int64', 'float', 'float64']).columns.tolist()
        for col in not_number_cols_raw:
            df[col] = pd.to_numeric(df[col], errors='ignore')

        not_number_cols = df.select_dtypes(exclude=['int', 'int64', 'float', 'float64']).columns.tolist()
        number_cols = df.select_dtypes(include=['int', 'int64', 'float', 'float64']).columns.tolist()
        self.columns_correction = list(set(not_number_cols_raw)-set(not_number_cols))
        ...
        for col in self.clean_col_names:
            if col in number_cols:
                cleaned_col_types.append('numerical')
